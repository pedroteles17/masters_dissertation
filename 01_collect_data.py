...
products_barcode = pd.read_excel(
    "data/best_sellers.xls", dtype=str
)["barcode"].tolist()[:10]
...
meli_products = []
for barcode in tqdm(products_barcode, total=len(products_barcode), desc="Searching products"):
    try:
...
meli_products = pd.DataFrame(meli_products)

#%%
buy_box_winners = []
for product_id in tqdm(meli_products["product_id"], total=len(meli_products["product_id"]), desc="Getting buy box winners"):
    try:
...
sellers_by_product = []
for product_id in tqdm(meli_products["product_id"], total=len(meli_products["product_id"]), desc="Getting sellers by product"):
    try:
...
sellers_info = []
for seller_id in tqdm(unique_sellers_ids, total=len(unique_sellers_ids), desc="Getting sellers info"):
...
sellers_info = sellers_info[
    [
        'id', 'nickname', 'country_id', 'user_type', 'site_id', 'permalink',
        'transactions_period', 'transactions_total', 'city', 'state',
        'level_id', 'power_seller_status', 'site_status'
    ]
]
...
sellers_by_product = sellers_by_product\
    .merge(sellers_info, on="seller_id", how="left")\
    .merge(buy_box_winners, on=["product_id", "seller_id"], how="left")\
...
sellers_by_product.to_parquet(
    f"data/sellers_by_product_{today}.parquet", 
    index=False
)
